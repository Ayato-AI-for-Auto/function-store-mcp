...
本システムは MVP（最小限の機能セット）として、**「保存された関数 A が別の保存された関数 B を呼び出す」といった関数間の依存解決は行いません。**

複雑な依存関係によるトラブル（Dependency Hell）を避けるため、以下の運用を推奨します：

- **Self-contained (自己完結)**: 依存し合う複数の関数がある場合は、それらを**1つの自己完結したコードブロック**として統合し、ひとつの関数（または「関数パック」名）として保存してください。
- **重複の許容**: 共通のユーティリティがある場合も、それぞれの関数に含めて保存することで、各資産のポータビリティ（独立してどこでも動く状態）を担保してください。
- **外部ライブラリ**: `pip` でインストール可能な外部ライブラリは `dependencies` 引数にリストアップしてください。

これにより、検索して取得したコードをそのままコピペするだけで、追加の依存解決なしに動作させることができます。
