...
## 6. 今後の拡張性・改善点 (Horiemon's Critique)

1.  **Flet DashboardのWebアプリ化**:
    *   現状はローカルアプリだが、FastAPI + React/Vue にして、チーム内共有サーバーとして動かせるようにすべき。Solo-MCPからTeam-MCPへの進化。
2.  **Dependency Hellの解決**:
    *   関数ごとの依存関係はメタデータにあるが、実行時にそれをどう解決するか (`pip install` を勝手に走らせるのか？) がまだ `runtime_docker.py` などの実験的実装に留まっている。ここを `uv` と連携して爆速で環境分離する仕組みが必要。
3.  **Vector Storeの汎用化**:
