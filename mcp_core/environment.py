...
class EnvManager:
    ...
    def get_python_executable(self, dependencies: List[str]) -> Tuple[str, str]:
        ...
        requested_set = set(d.split('==')[0].split('>=')[0].strip().lower() for d in dependencies)
        for name, deps in BASE_ENV_CONFIGS.items():
            ...
            if requested_set.issubset(base_set):
                ...
                env_hash = f"base_{name}_{hashlib.sha256(deps_str.encode()).hexdigest()[:8]}"
        ...
        logger.info(f"EnvManager: Creating new environment for {dependencies} at {env_path}")
    ...
    def _create_env(self, env_path: Path, dependencies: List[str]) -> Tuple[str, str]:
        ...
        python_exe = env_path / ("Scripts/python.exe" if os.name == "nt" else "bin/python")
        try:
            subprocess.run(["uv", "venv", str(env_path)], check=True, capture_output=True, timeout=60)
            install_cmd = ["uv", "pip", "install", "--python", str(python_exe)] + dependencies
            ...
            while True:
                ...
                if time.time() - start_time > timeout:
                    ...
                    return "", f"Install Timeout: Dependency installation took longer than {timeout}s."
