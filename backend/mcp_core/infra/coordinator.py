...
import httpx
from mcp_core.core.config import PORT
...
class Coordinator:
    ...
    def __init__(self):
        # Always use localhost for loopback communication on Windows
        self.master_url = f"http://127.0.0.1:{MASTER_PORT}"
    ...
    def is_master_running(self) -> bool:
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            s.settimeout(0.5)
            try:
                # Use 127.0.0.1 for connection check regardless of HOST binding
    ...
    def start_master_invisible(self):
        ...
        cmd = [sys.executable, "-m", "mcp_core.infra.background_server"]
